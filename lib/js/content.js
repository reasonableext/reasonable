(function() {
  var ARTICLE_REGEX, ARTICLE_SHORTEN_REGEX, AVATAR_PREFIX, AVATAR_SUFFIX, COLLAPSE, COMMENT_HISTORY, ESCAPE_KEY, FADE_SPEED, FIRST_MATCH, IGNORE, LIGHTS_OUT_OPACITY, MINIMAL_FADE_SPEED, MINIMAL_OPACITY, MY_MD5, PICTURE_REGEX, QUICKLOAD_MAX_ITEMS, QUICKLOAD_SPEED, TOTAL_OPACITY, UNCOLLAPSE, UNIGNORE, UPDATE_POST_TIMEOUT_LENGTH, URL_REGEX, YOUTUBE_REGEX, altText, blockTrolls, buildQuickInsert, buildQuickload, commentOnlyRoutines, defaultSettings, formatDate, getLink, getName, getSettings, gravatars, historyAndHighlight, lightsOn, lightsOut, months, quickInsert, removeGooglePlus, settings, showImagePopup, showMedia, trolls, updatePosts, viewThread;
  ...
  blockTrolls = function(smoothTransitions) {
    ...
    return $($("h2.commentheader strong")).each(function() {
      ...
      if (__indexOf.call(settings.trolls, name) >= 0 || (link !== "" && __indexOf.call(settings.trolls, link) >= 0)) {
        ...
        return $this.siblings("a.ignore").text(IGNORE).closest("div").removeClass("troll").children("p, blockquote").slideDown();
      }
    });
  };
  showMedia = function() {
        ...
        default:
          switch (key) {
            case "trolls":
              ...
              temp.trolls = {};
              return $.each(arr, function(trollKey, trollValue) {
            ...
            case "history":
              try {
                return temp.history = JSON.parse(temp.history).sort(function(a, b) {
