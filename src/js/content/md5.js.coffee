class MD5
  ...
  @rotateLeft: (value, shiftBits) ->
    (value << shiftBits) | (value >>> (32 - shiftBits))
  ...
  @addUnsigned: (x, y) ->
    ...
    return (result ^ 0x80000000 ^ x8 ^ y8) if (x4 & y4)
    if (x4 | y4)
  ...
  @convertToWordArray: (string) ->
    ...
    numberOfWords_temp1 = messageLength + 8
    numberOfWords_temp2 = (numberOfWords_temp1 - (numberOfWords_temp1 % 64)) / 64
    numberOfWords = (numberOfWords_temp2 + 1) * 16
    wordArray = Array(numberOfWords-1)
    bytePosition = 0
    byteCount = 0
